...
`a0` (default -0.1; the baseline must leave room for `beta * z_j` before any node destabilises the linear model),
`beta` (search grid; replaces `search.a`, which is ignored in this mode), `clip`, `zscore`.  Ready-made T1w/T2w myelin tables for the
